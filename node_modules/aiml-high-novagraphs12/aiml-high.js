...
/**
 * Rimuove i caratteri di controllo da una stringa, come ad esempio i ritorni a capo ('\r\n'),
 * gli spazi iniziali e finali, ecc.
 *
 * @param {string} str - La stringa da pulire.
 * @return {string} - La stringa pulita senza i caratteri di controllo.
 */
var cleanStringFormatCharacters = function(str) {
    // Rimuovi i ritorni a capo dalla stringa
...
/**
 * Funzione che pulisce l'array di nodi figli (childNodes) dall'albero DOM.
 * Rimuove i nodi di testo vuoti contenenti solo '\r\n', che indicano una riga vuota nel file AIML.
 * Operando ricorsivamente, attraversa l'intero albero DOM.
 *
 * @param {Array} childNodes - Array di nodi figli da pulire.
 * @return {Array} - Array di nodi figli eventualmente modificato dopo la pulizia.
 */
...
/**
 * Trova la categoria corretta nel documento AIML in base all'input del cliente.
 *
 * @param {string} clientInput - L'input fornito dal cliente.
 * @param {Array} domCategories - Array di categorie nel documento AIML.
 * @return {string|null} - Il testo corrispondente alla categoria trovata o null se nessuna corrisponde.
 */
var findCorrectCategory = function(clientInput, domCategories) {
    ...
    /**
     * Funzione interna per attraversare il DOM alla ricerca del pattern corrispondente.
     *
     * @param {Array} categories - Array di categorie nel documento AIML.
     * @return {string|null} - Il testo corrispondente al pattern trovato o null se nessuno corrisponde.
     */
    ...
    /**
     * Verifica se esiste un tag 'that' nella categoria e se il suo testo corrisponde alla risposta precedente.
     *
     * @param {Array} categoryChildNodes - Array di nodi figli della categoria nel documento AIML.
     * @return {boolean} - Restituisce true se il tag 'that' è presente e il suo testo corrisponde alla risposta precedente, altrimenti restituisce false.
     */
    ...
    /**
     * Risolve i nodi figli di un nodo di tipo 'pattern', generando una stringa risultante.
     * Supporta i tag 'bot', 'get', e 'set'.
     *
     * @param {Array} patternChildNodes - Array di nodi figli del nodo 'pattern' nel documento AIML.
     * @return {string} - La stringa risultante dopo la risoluzione dei nodi figli.
     */
    ...
    /**
     * Trova il testo finale nel nodo di template, risolvendo eventuali nodi speciali come 'bot', 'set', 'get', ecc.
     * 
     * @param {Array} childNodesOfTemplate - Array di nodi figli del nodo di template nel documento AIML.
     * @return {string} - Il testo finale dopo la risoluzione di nodi speciali.
     */
     ...
     * 
     * @param {Array} nodeContent - contenuto del tag. 
     * @return {string} - Il contenuto del tag.
     */
    var getContent = function(nodeContent) {
        var content = 'undefined';
        
        if (nodeContent.length === 1) {
            content = nodeContent[0].nodeValue;
        }
    ...
    /**
     * Risolve i nodi speciali come 'bot', 'set', 'get', ecc., generando una stringa risultante.
     * 
     * @param {Array} innerNodes - Array di nodi figli del nodo speciale nel documento AIML.
     * @return {string} - La stringa risultante dopo la risoluzione di nodi speciali.
     */
    var resolveSpecialNodes = function(innerNodes) {
        ...
        for (var i = 0; i < innerNodes.length; i++) {
            if (innerNodes[i].tagName === 'bot') {
                ...
                } else if (innerNodes[i].childNodes[0].nodeValue === '*' || innerNodes[i].childNodes[0].nodeValue === '_') {
              ...
              text = text + formalText.split(' ').map(function(str) {
                ...
                //get attribute index of star tag
                var index = innerNodes[i].getAttribute('index');
                text = text + wildCardArray[index - 1];
            } else if (innerNodes[i].tagName === 'formula') {
                var formula = getContent(innerNodes[i].childNodes);
                text = text + manageFormula(formula);
            } else if (innerNodes[i].tagName === 'image') {
                var image = getContent(innerNodes[i].childNodes);
                text = text + manageImage(image);
            } else if (innerNodes[i].tagName === 'svgElement') {
                var id_element = getContent(innerNodes[i].childNodes);
                var style_name = innerNodes[i].getAttribute('style-name');
                var style_value = innerNodes[i].getAttribute('style-value');

                text = text + manageSvgElement(id_element, style_name, style_value);
                ...
                } else if (innerNodes[i].hasAttribute('value')) { // condition tag specification: multi condition tag
                    if (storedVariableValues[innerNodes[i].getAttribute('name')] === innerNodes[i].getAttribute('value')) {
                        text = text + resolveSpecialNodes(innerNodes[i].childNodes);
                    }
                } else if (innerNodes[i].childNodes.length > 0) { // condition tag specification: single name list condition tags
                    var child;
                    for (var c in innerNodes[i].childNodes) {
                        child = innerNodes[i].childNodes[c];
                        if (child.tagName === 'li') {
...
/**
 * Verifica se il messaggio dell'utente corrisponde al testo del pattern. 
 * 
 * @param {string} userInput - L'input fornito dall'utente.
 * @param {string} patternText - Il testo del pattern da confrontare con l'input dell'utente.
 * @return {boolean} - Restituisce true se c'è una corrispondenza, altrimenti false.
 */
...
/**
 * Converte il carattere jolly nel testo in una regex che corrisponde a ogni carattere.
 * 
 * @param {string} text - Il testo contenente eventuali caratteri jolly.
 * @return {string} - Il testo modificato con la regex al posto dei caratteri jolly.
 */
var convertWildcardToRegex = function(text) {
    ...
    var modifiedText = text.replace(/ _/g, '*').replace(/ \*/g, '*');
...
/**
 * Ottiene il valore delle wildcard nella stringa di input utente basato sul testo del pattern.
 * 
 * @param {string} userInput - L'input fornito dall'utente.
 * @param {string} patternText - Il testo del pattern contenente wildcard.
 * @return {Array} - Un array contenente i valori delle wildcard nella stringa di input utente.
 */
...
var manageFormula = function(formula) {
    return '\\(' + formula + '\\)';
}

var manageImage = function(image) {
    return 'figure{' + image + '} ';
}

var manageSvgElement = function(id_element, style_name, style_value) {
    return 'element{#' + id_element + '--' + style_name + '--' + style_value + '}';
}
